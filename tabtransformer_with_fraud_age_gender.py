...
# set seaborn style because it is prettier
sns.set()
...
def train_model(config):
    ...
    if config.algo == 'xgboost':
    ...
    elif config.algo == 'tabtransformer':
        torch.manual_seed(1)
        np.random.seed(1)
        random.seed(1)
        ...
        model = TabTransformer(dls.classes, dls.cont_names, 2, d_v=32, d_k=32)
        ...
        # pos_weight = 1.0 if config.do_smote else float(np.sqrt(pos_weight))
        pos_weight = float(np.sqrt(pos_weight))
        loss_func = fastai.losses.CrossEntropyLossFlat(weight=torch.tensor([1.0, pos_weight], device=device))
        # loss_func = fastai.losses.FocalLossFlat(weight=torch.tensor([1.0, pos_weight], device=device))
        ...
        print("Confusion Matrix of TabTransformer: \n", confusion_matrix(y_test, final_preds))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--algo", default="tabtransformer", type=str, help="algorithm name",
                        choices=['tabtransformer', 'xgboost', 'lightgbm'])
